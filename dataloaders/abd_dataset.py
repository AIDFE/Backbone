...
import math
import itertools
from .normalize_utils import get_normalize_op

hostname = platform.node()
# folder for datasets
BASEDIR = '/mnt8t/lijiaxi/Datasets/Abdominal'
...
class AbdominalDataset(torch_data.Dataset):
    def __init__(self,  mode, transforms, base_dir, domains: list,  idx_pct = [0.7, 0.1, 0.2], tile_z_dim = 3, extern_norm_fn = None):
        ...
        if extern_norm_fn is None:
            self.normalize_op = get_normalize_op(self.domains[0][:2], [itm['img_fid'] for _, itm in self.sample_list[self.domains[0]].items()])
...
tr_func  = trans.transform_with_label(trans.my_aug)
